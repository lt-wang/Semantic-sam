...
def parse_option():
    ...
    parser.add_argument('--ckpt',default="./ckpts/swinl_only_sam_many2many.pth", metavar="FILE", help='path to ckpt',dest='ckpt')
...
demo = gr.Blocks()
image=ImageMask(label="Click on Image (Please only click one point, or our model will take the center of all points as the clicked location. Remember to clear the click after each interaction, or we will take the center of the current click and previous ones as the clicked location.)",type="pil",height=512)
text_model_level=gr.Textbox(label="Output levels",value="0,1,2,3,4,5",visible=True)
text_model_select=gr.Radio(['Prompt 1', 'Prompt 2', 'Prompt 3', 'Prompt 4', 'Prompt 5', 'Prompt 6', 'All Prompt',],value='All Prompt', label="Our model learns 6 granularity prompts. [1-6] indicates output granularity from largest to smallest using different prompts.  'All prompt' means using all 6 granularity prompts.")
image_out=gr.Image(label="Auto generation",type="pil",height=512)
...
demo.queue().launch(share=False,server_port=6081)
