...
def interactive_infer_image(model, image,all_classes,all_parts, thresh,text_size,hole_scale,island_scale,semantic, refimg=None, reftxt=None, audio_pth=None, video_pth=None):
    ...
    # Support both old gradio dict format and new ImageEditor format
    if 'background' in image:
        img_pil = image['background'].convert('RGB')
        layers = image.get('layers', [])
        if layers and layers[0] is not None:
            mask_pil = layers[0].convert('RGBA')
        else:
            mask_pil = Image.new('RGBA', img_pil.size, (0, 0, 0, 0))
    else:
        img_pil = image['image']
        mask_pil = image['mask']
    image_ori = transform1(img_pil)
    mask_ori = transform1(mask_pil)
    ...
    mask_arr = np.asarray(mask_ori)
    # RGBA (ImageEditor layers) -> use alpha channel; RGB -> use first channel
    if mask_arr.ndim == 3 and mask_arr.shape[2] == 4:
        mask_arr = mask_arr[:, :, 3:4].copy()
    else:
        mask_arr = mask_arr[:, :, 0:1].copy()
    mask_ori = torch.from_numpy(mask_arr).permute(2,0,1)[0]
