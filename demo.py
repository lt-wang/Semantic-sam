...
class ImageMask(gr.ImageEditor):
    """
    Uses gr.ImageEditor for sketch/drawing interaction (gradio 6.x compatible)
    """
...
demo = gr.Blocks()
image=ImageMask(label="Click on Image (Please only click one point, or our model will take the center of all points as the clicked location. Remember to clear the click after each interaction, or we will take the center of the current click and previous ones as the clicked location.)",height=512)
gallery_output=gr.Gallery(label="Image Gallery sorted by IoU score.",min_width=1536,columns=6)
gallery_output2=gr.Gallery(label="Image Gallery sorted by mask area.",min_width=1536,columns=6)
text=gr.Textbox(label="Categories. (The default is the categories in COCO panoptic segmentation.)",value=":".join(all_classes),visible=False)
text_part=gr.Textbox(label="Part Categories. (The default is the categories in PASCAL Part.)",value=":".join(all_parts),visible=False)
text_res=gr.Textbox(label="\"class:part(score)\" of all predictions (seperated by ;): ",visible=True)
text_thresh=gr.Textbox(label="The threshold to filter masks with low iou score.",value="0.5",visible=True)
text_size=gr.Textbox(label="image size (shortest edge)",value="640",visible=True)
hole_scale=gr.Textbox(label="holes scale",value="100",visible=True)
island_scale=gr.Textbox(label="island scale",value="100",visible=True)
text_model_size=gr.Textbox(label="model size (L or T)",value="L",visible=True)
text_ckpt=gr.Textbox(label="ckpt path (relative to /mnt/output/)",value="fengli/joint_part_idino/train_interactive_all_m2m_swinL_bs16_0.1part9_nohash_bs1_resume_all_local_0.15_onlysa_swinL_4node_mnode/model_0099999.pth",visible=True)
text_ckpt_now=gr.Textbox(label="current ckpt path (relative to /mnt/output/)",value="",visible=True)
...
demo.queue().launch(share=False,server_port=6082)
